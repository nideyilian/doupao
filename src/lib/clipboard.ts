...
export async function copyImageSourceToClipboard(src: string | Promise<string | undefined>) {
  ...
  const { isElectron } = await import('./localSave')
  if (isElectron() && window.electronAPI?.writeImageToClipboard) {
    const ok = await window.electronAPI.writeImageToClipboard(resolvedSrc)
    if (ok) return
