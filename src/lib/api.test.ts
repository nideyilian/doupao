...
describe('callImageApi', () => {
  ...
  it('sends a single Images API streaming request when n is greater than 1', async () => {
    ...
    expect(result.images).toEqual(['data:image/png;base64,ZmluYWw=', 'data:image/png;base64,ZmluYWw='])
  ...
  it('uses the same-origin API proxy path for sync custom providers', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api-proxy/custom/images', expect.objectContaining({ method: 'POST' }))
  })
  ...
  it('sends size=auto on Images API edit requests regardless of the selected size', async () => {
    ...
    await callImageApi({
      ...
      inputImageDataUrls: ['data:image/png;base64,iVBORw0KGgo='],
    })

    const [url, init] = fetchMock.mock.calls.find(([u]) => typeof u === 'string' && u.includes('/images/edits')) ?? []
  ...
  it('sends size=auto on Responses API image tools when reference images are present', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(
        JSON.stringify({
